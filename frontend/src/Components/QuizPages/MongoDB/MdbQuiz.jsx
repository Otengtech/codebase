...
const MdbQuiz = () => {
  ...
  const [topScore, setTopScore] = useState(
    parseInt(localStorage.getItem("mdbTopScore")) || 0
  );

  useEffect(() => {
    // Load cached questions instantly if available
    const cachedQuestions = localStorage.getItem("mdbQuizQuestions");
    if (cachedQuestions) {
      setQuestions(JSON.parse(cachedQuestions));
      setLoading(false);
    }

    // Always fetch fresh questions in background
    fetch(`${API_URL}/api/mdb-quiz`)
      ...
      .then((data) => {
        setQuestions(data);
        localStorage.setItem("mdbQuizQuestions", JSON.stringify(data));
        setLoading(false);
      })
      .catch((err) => {
